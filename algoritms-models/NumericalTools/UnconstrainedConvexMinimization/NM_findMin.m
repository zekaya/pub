function [ minVal ] = NM_findMin(inVect, hessian, b, c, predefinedStepSize)
...
while(dx>tol && count < maxIterationNum) %check for termination rule
    ...
    if predefinedStepSize > 0 %Calculate the step size if it's not provided
        ...
        stepSize = NM_stepSize(currentGrad,hessian);
    end
    
    nextPoint = NM_nextPoint(currentPoint, stepSize, currentGrad ,hessian);
    dx= norm(currentGrad);
    count = count + 1;
    currentVal = subs(E,[t1 t2], currentPoint');
